...
def extract_latest_day(html_content: str) -> tuple:
    ...
    after = html_content[last_pos:]
    
    # 找到 journey-item 的结束位置：</div> 后面跟 3个换行+<footer>
    # 实测确认：pattern 在 after[3550:3567] 处匹配
    close_pat = re.compile(r'</div>(\n\s*)+<footer>')
    m = close_pat.search(after)
    if not m:
        raise ValueError("未找到 journey-item 结束标记")
    
    end_pos = m.start() + 6  # include the </div>
    item_text = after[:end_pos]
    ...
    latest_title = title_m.group(1).strip() if title_m else f"AI环游世界 Day {day_comments[-1].group(1)}"
...
def main():
    print(f"\n{'='*60}")
    print(f"WordPress发布 - AI环游世界 最终版")
